...
if (isset($_REQUEST['loan_id'])) {
    $accid = $_REQUEST['loan_id'];

    // Use prepared statement
    $stmt = $connection->prepare("SELECT * FROM loans WHERE loan_id = ?");
...
<body>
    <form method="POST">
        ...
        <label for="interest_rate">value:</label>
        <input type="number" name="interest_rate" value="<?php echo isset($d) ? htmlspecialchars($d, ENT_QUOTES) : ''; ?>" required>
        <br><br>
...
if (isset($_POST['up'])) {
    ...
    $interest_rate = htmlspecialchars($_POST['interest_rate'], ENT_QUOTES);

    // Use prepared statement for update
    $stmt = $connection->prepare("UPDATE loans SET account_id = ?, amount= ?,interest_rate = ? WHERE loan_id = ?");
    $stmt->bind_param("sssi", $account_id, $amount, $interest_rate, $accid);
